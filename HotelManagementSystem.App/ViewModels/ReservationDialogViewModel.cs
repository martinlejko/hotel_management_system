...
{
    public class ReservationDialogViewModel : ViewModelBase
    {
        ...
        public ReservationDialogViewModel(Window dialog, DbContextOptions dbOptions, Reservation? reservation = null)
        {
            _dialog = dialog;
        ...
        private async Task LoadCustomersAsync()
        {
            using (var context = new HotelDbContext((DbContextOptions<HotelDbContext>)_dbOptions))
            {
                ...
                if (_originalReservation != null && _originalReservation.Customer != null)
                {
        ...
        private async Task LoadAvailableRoomsAsync()
        {
            using (var context = new HotelDbContext((DbContextOptions<HotelDbContext>)_dbOptions))
            {
                ...
                if (_originalReservation != null && _originalReservation.Room != null)
                {
                    // For editing, always include the original room in the list
                    if (_originalReservation.Room != null && 
                        !AvailableRooms.Any(r => r.Id == _originalReservation.RoomId))
                    {
        ...
        private void Save()
        {
            ...
            // Close the dialog with the reservation as result
            _dialog.Close(reservation);
        }
